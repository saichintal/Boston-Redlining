...
def get_neighborhood_by_zipcode(zipcode): 
    for neighborhood, value in NEIGHBORHOODS.items():
        # converts list ofstring to list of ints 
...
def get_avg_poverty_rate(df):
    ...
    for i in df.index:
        ...
        if area in area_poverty_number:
            ...
            area_poverty_number[area] = {'population':
                                         curr_siz + num_of_low_income_people}
        else:
    ...
    for neighborhood, value in NEIGHBORHOODS.items():
        ...
        if neighborhood in area_poverty_number:
            ...
            area_poverty_number[neighborhood]['rate'] =(pov_pop /
                                                        neighborhood_pop) * 100
            neighborhood_code = NEIGHBORHOODS[neighborhood]['code']
            area_poverty_number[neighborhood]['code'] = neighborhood_code
...
def get_average_grad_rate(df):
    ...
    for i in df.index:
        ...
        if neighborhood in neighborhood_avg_grad_rate:
            ...
            average_value = {'rate': ((curr_avg * curr_size) + graduation_rate)
                             / (curr_size + 1),
                            'size': curr_size + 1,
                            'code': NEIGHBORHOODS[neighborhood]['code']}
            neighborhood_avg_grad_rate[neighborhood] = average_value
        else:
...
def get_average_property_val(df):
    ...
    for i in df.index:
        ...
        if neighborhood in neighborhood_avg_property_val:
            ...
            average_value = {'rate': ((curr_avg * curr_size) + property_value)
                             / (curr_size + 1),
                            'size': curr_size + 1,
                             'code': NEIGHBORHOODS[neighborhood]['code']}
            neighborhood_avg_property_val[neighborhood] = average_value 
        else:
...
def get_avg_value_for_each_code(data): 
    ...
    for neighborhood, value in data.items(): 
        ...
        average_value = ((curr_avg *  curr_size) + rate) / (curr_size + 1)
        avg_code_value[code]['avg'] =  average_value
...
def neighborhood_bar_chart(neighborhood_data, y_value_title, chart_title):
    ...
    #sorts the lists from largest to smallest for a better graph
...
def sort_lists( rates, neighborhoods):
    ...
    could be done with a premade function but didn't want to delete the work.
    '''
...
def main():
    ...
    average = get_avg_value_for_each_code(neighborhood_avg_graduation_rate)
    avg_graduation_for_code = average

    #Poverty Rates
    df2 = pd.read_excel(POC2_DATASET,
                        sheet_name='Climate_Ready_Boston_Social_Vul')
    neighborhood_avg_poverty_rate = get_avg_poverty_rate(df2)
    average = get_avg_value_for_each_code(neighborhood_avg_poverty_rate)
    avg_poverty_for_code = average
    
    #Property Values
    df = pd.read_excel(PROPERTY_DATASET, sheet_name='Sheet1')
    neighborhood_avg_property_value = get_average_property_val(df)    
    average = get_avg_value_for_each_code(neighborhood_avg_property_value)
    avg_property_value_for_code = average
    ...
    averages_bar_chart(avg_graduation_for_code,
                       "graduation rate",
                       "Graduation Rate by Code")
    #average poverty rate by code
    averages_bar_chart(avg_poverty_for_code,
                       "poverty rate",
                      "Poverty Rate by Code")
    #average property values by code
    averages_bar_chart( avg_property_value_for_code,
                        "property value",
                        "Average Property Value by Code")
    #graduation rates by neighborhood
    neighborhood_bar_chart( neighborhood_avg_graduation_rate,
                            "Graduation Rate",
                            'Graduation Rates by Neighborhood')
    #poverty Rates by Neighborhood
    neighborhood_bar_chart( neighborhood_avg_poverty_rate,
                            "Poverty Rate",
                            'Poverty Rates by Neighborhood')
    #Average Property Values by Neighborhood
    neighborhood_bar_chart(neighborhood_avg_property_value,
                           "Average Property",
                           'Average Property Values by Neighborhood')
